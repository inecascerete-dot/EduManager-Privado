    ...
    if seleccion:
        ...
        if resultado_per:
                ...
                if foto_url and foto_url != "sin_foto.png":
                    ...
                    try:
                        ...
                        st.info(
                            ...
                            f"(Falta cargar el archivo físico en el servidor)"
                        )

                else:
                    st.warning(
                        "👤 Este funcionario no tiene una fotografía registrada en el sistema."
                    )
            ...
            with col4:
                ...
                st.markdown(
                    f"**Estado Laboral Actual:** `{estado_lab}` ({f_estado})"
                )
...
# --- 🆕 PANTALLA: REGISTRAR PERSONAL ---
    ...
    # 📋 SECCIÓN 1: DATOS DE IDENTIFICACIÓN Y PERFIL
    ...
    # 📸 SECCIÓN 3: GESTIÓN DE FOTOGRAFÍA (Subida de archivo)
    st.markdown("### 📸 3. Gestión de Fotografía")
    archivo_foto = st.file_uploader("Subir foto de perfil del docente/directivo (Formatos JPG, PNG)", type=["jpg", "jpeg", "png"])
    
    # Definimos el nombre que irá a la base de datos
    foto_url_valor = "sin_foto.png"
    if archivo_foto is not None:
        # Para evitar que dos fotos se llamen igual, le prependemos el ID del personal (Ej: PERS-005_foto.png)
        extension = archivo_foto.name.split(".")[-1]
        foto_url_valor = f"{per_id.strip()}_foto.{extension}"

    st.write("---")
    
    # 💼 SECCIÓN 4: SITUACIÓN LABORAL Y ESTATUTO DOCENTE
    ...
    if boton_guardar_per:
        ...
        if not (id_p_l and doc_l and nom_l and ape_l and tel_l and correo_l):
            ...
            with st.spinner("Registrando funcionario y guardando archivos..."):
                ...
                # --- 💾 PASO CLAVE: Guardar el archivo físico en la carpeta 'assets' del servidor ---
                if archivo_foto is not None:
                    # Creamos la carpeta 'assets' si no existe en tu carpeta del proyecto
                    if not os.path.exists("assets"):
                        os.makedirs("assets")
                    
                    # Ruta completa donde se va a escribir (Ej: assets/PERS-005_foto.png)
                    ruta_destino = os.path.join("assets", foto_url_valor)
                    
                    # Escribimos los bytes binarios de la imagen en el disco duro
                    with open(ruta_destino, "wb") as f:
                        f.write(archivo_foto.getbuffer())
                
                # Enviamos los datos ordenados a PostgreSQL incluyendo el nombre final de la foto
